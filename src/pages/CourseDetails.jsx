...
export default function CourseDetails() {
  ...
  useEffect(() => {
    const fetchCourseData = async () => {
      ...
      try {
        ...
        setAllCourses(snapshot.docs.map(d => ({ id: d.id, ...d.data() })).filter(c => c.id !== id));
      } catch (error) {
  ...
  if (loading) return <p className="text-center mt-4 text-sm">Loading course details...</p>;
  ...
  if (!course)
    return (
      <div className="text-center py-6">
        ...
        <Button text="Back to Courses" variant="primary" size="md" onClick={() => navigate("/courses")} />
      </div>
    );
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8">
      ...
      <HeroSection
        ...
        description="Learn from the best instructors and grow your career."
      ...
      <div className="max-w-5xl mx-auto p-4 mb-4">
        {course.videoUrl ? (
          ...
          <img src={course.imageUrl} alt={course.title} className="w-full h-56 md:h-80 object-cover rounded-xl mb-4" />
        ) : (
      ...
      <HeroSection
        ...
        className="min-h-[40vh]"
        buttons={[{ text: "Back to Courses", variant: "white", size: "lg", shape: "pill", onClick: () => navigate("/courses") }]}
      />
